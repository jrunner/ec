...
<%@include file="../../common/init.jsp"%>
	...
	<script type="text/javascript">
	 ...
	 function save(){
		 ...
		 JSendAjax('<%=contextPath%>/baseinfo/vipunit.do?cmd=updateSub',data);
	 }
     ...
     function JCollBack(_data,_textStuats){
         ...
         if(info == '0'){
             if(msg=='ok'){
                 ...
                 if(state!='true'){
	                 //这里的id作为code使用
		             onAddNode(id,id,'<%=request.getParameter("pcode") %>','/demo/baseinfo/vipunit.do?cmd=toSubList',resname,'<%=request.getParameter("treeid") %>','subinstry') ;
                 }
                 if(state=='true'){
						...
						<tr>
							...
							<td class="td_global">
								<table width="99%" border="0" align="center" cellpadding="0" cellspacing="1" class="table_content">
									...
									<tr>
										<td class="table_content_td"  width="15%">行业名称：</td>
									...
									<tr>
										<td class="table_content_td">重点行业：</td>
										<td class="table_content_td_left">
											<input type="checkbox"/>
										</td>
									</tr>  
